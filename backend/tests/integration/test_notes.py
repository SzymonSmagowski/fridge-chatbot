...
def test_get_notes_returns_pinned_first(
    ...
    client.post(
        "/api/notes", headers=auth_headers, json={"content": "older", "pinned": False}
    )
    client.post(
        "/api/notes",
        headers=auth_headers,
    ...
    listed = client.get("/api/notes", headers=auth_headers).json()
...
def test_patch_note_updates_content(
    ...
    note = client.post(
        "/api/notes", headers=auth_headers, json={"content": "old"}
    ).json()
    resp = client.patch(
        f"/api/notes/{note['id']}", headers=auth_headers, json={"content": "new"}
    )
...
def test_delete_note_hard_deletes(
    ...
    note = client.post(
        "/api/notes", headers=auth_headers, json={"content": "to delete"}
    ).json()
...
def test_get_note_in_other_family_returns_404(
    ...
    resp = client.get(f"/api/notes/{other_note['id']}", headers=auth_headers)
...
def test_get_notes_filtered_by_label_returns_only_matching(
    ...
    resp = client.get("/api/notes?label=shopping-list", headers=auth_headers).json()
...
async def test_post_note_invalidates_notes_cache(
    ...
    assert primed
    client.post("/api/notes", headers=auth_headers, json={"content": "new"})
...
async def test_post_note_publishes_note_created_event(
    ...
    async with family_event_collector(family_id) as collector:
        client.post("/api/notes", headers=auth_headers, json={"content": "test"})
...
@pytest.mark.asyncio
async def test_shopping_list_append_publishes_note_event(
    client: TestClient, auth_headers, family, family_event_collector
) -> None:
    """Append goes through NoteService.create (auto-create branch when no
    shopping-list note exists) or .update (existing list, line added). Both
    publish standard `note.created` / `note.updated` frames — the service
    layer doesn't emit a bespoke `note.shopping_list.appended` type."""
    family_id, _, _ = family
    async with family_event_collector(family_id) as collector:
        client.post(
            "/api/notes/shopping-list/append",
        ...
        frames = await collector.wait_for(1)
    # First call (no existing list note) takes the auto-create branch ⇒ note.created.
    assert frames[0]["type"] in {"note.created", "note.updated"}
    assert frames[0]["entity"] == "notes"
