...
def test_get_cars_lists_only_active_by_default(
    ...
    client.post("/api/cars", headers=auth_headers, json={"name": "Volvo"})
    b = client.post("/api/cars", headers=auth_headers, json={"name": "Civic"}).json()
    client.post(f"/api/cars/{b['id']}/set-inactive", headers=auth_headers)
    listed = client.get("/api/cars", headers=auth_headers).json()
...
def test_patch_car_updates_fields(
    ...
    car = client.post("/api/cars", headers=auth_headers, json={"name": "Volvo"}).json()
    resp = client.patch(
        f"/api/cars/{car['id']}",
        headers=auth_headers,
...
def test_set_inactive_then_active_round_trip(
    ...
    car = client.post("/api/cars", headers=auth_headers, json={"name": "Civic"}).json()
    inactive = client.post(
        f"/api/cars/{car['id']}/set-inactive", headers=auth_headers
    ).json()
    assert inactive["status"] == "inactive"
    active = client.post(
        f"/api/cars/{car['id']}/set-active", headers=auth_headers
    ).json()
...
def test_delete_car_hard_deletes_row(
    ...
    car = client.post("/api/cars", headers=auth_headers, json={"name": "Civic"}).json()
    resp = client.delete(f"/api/cars/{car['id']}", headers=auth_headers)
...
def test_post_car_with_empty_name_returns_422(
    ...
    resp = client.post("/api/cars", headers=auth_headers, json={"name": ""})
...
async def test_delete_car_invalidates_notes_and_events_caches(
    ...
    car = client.post("/api/cars", headers=auth_headers, json={"name": "Civic"}).json()
    # Prime three caches
    client.get("/api/cars", headers=auth_headers)
    ...
    client.delete(f"/api/cars/{car['id']}", headers=auth_headers)
