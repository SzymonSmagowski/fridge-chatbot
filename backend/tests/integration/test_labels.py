...
def test_post_label_with_empty_slug_returns_422(
    ...
    resp = client.post(
        "/api/labels", headers=auth_headers, json={"slug": "", "display_name": "X"}
    )
...
def test_get_labels_returns_list_with_note_counts(
    ...
    client.post(
        "/api/notes", headers=auth_headers, json={"content": "x", "label_slugs": ["todo"]}
    )
    client.post(
        "/api/notes", headers=auth_headers, json={"content": "y", "label_slugs": ["todo"]}
    )
    resp = client.get("/api/labels", headers=auth_headers)
...
def test_patch_label_updates_display_name(
    ...
    resp = client.patch(
        "/api/labels/todo", headers=auth_headers, json={"display_name": "Tasks"}
    )
...
def test_delete_user_label_returns_204(
    ...
    resp = client.delete("/api/labels/todo", headers=auth_headers)
...
def test_delete_reserved_shopping_list_label_returns_409(
    ...
    resp = client.delete("/api/labels/shopping-list", headers=auth_headers)
