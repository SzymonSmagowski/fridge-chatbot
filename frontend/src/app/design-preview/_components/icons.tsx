...
/**
 * Curated icon set for notes (per notes.md §Constraints — name stored as
 * string, resolved to a React element at render time).
 *
 * The final curated list is Designer's responsibility; Architect / Developer
 * should ship the same name -> element mapping on the backend validator.
 *
 * Returning a ReactElement rather than a component avoids the
 * react-hooks/static-components lint error from dynamically-looked-up tags.
 */
...
export function renderNoteIcon(
  name: NoteIconName | string | undefined,
  size = 18,
): ReactElement | null {
  switch (name) {
    case "shopping-cart":
