 ...
 * `POST /api/pairing/start`, receives an `authorize_url`, and navigates the
 * browser to it. Google then redirects to `GET /oauth/google/callback`
 * which 302s back to `/pair/complete?token=<device-jwt>` — the frontend
 * persists the token and lands on the home shell.
 */
...
test.describe("Pairing", () => {
  ...
  test("[e2e] start button POSTs /api/pairing/start and redirects to authorize_url", async ({ page }) => {
    const fakeAuthUrl =
      "http://localhost:3000/__stubbed_google__"; // same-origin so navigation actually completes
    await page.route(`${API}/api/pairing/start`, async (route) => {
